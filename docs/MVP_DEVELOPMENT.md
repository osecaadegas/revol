...
6. Responsible users can update task states and submit private photo evidence.
7. Managers can approve or reject submitted tasks.
8. The company can review the audit history.
...
| Evidence and validation | task drawer | `POST /api/tasks/:id/evidence`, `POST /api/tasks/:id/decision`, `GET /api/evidence/:id/file` |
...
- Submitting a task for validation requires at least one evidence file.
