...
On launch, visitors see the MANIFESTO client access area. The public marketplace remains available with filters for keyword, company, work position, location, and kilometer radius. Workers register to apply. Companies register to publish vacancies and review applications. Company/manager accounts use the operational workspace for user management, work orders, tasks, evidence upload, validation flow, and audit log. Client/developer accounts see the private project dashboard and MVP development cockpit.
